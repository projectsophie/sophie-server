...
const (
	...
	PagesPrepare       = "CREATE TABLE IF NOT EXISTS pages (id INTEGER PRIMARY KEY, workspace INTEGER NOT NULL, author INTEGER NOT NULL, creation_date TEXT NOT NULL, last_edited_date TEXT NOT NULL DEFAULT 'NA', last_editor INTEGER NOT NULL DEFAULT -1, type TEXT NOT NULL, content TEXT);"
	...
	GetPageById          = "SELECT * FROM pages WHERE id = @id"
	GetWorkspaceByPageId = "SELECT workspace FROM pages WHERE id = @id"
	DeletePageById       = "DELETE FROM pages WHERE id = @id"

	GetCommentById    = "SELECT * FROM comments WHERE id = @id"
